...
use crocksdb_ffi::{self, DBBackupEngine, DBCFHandle, DBCompressionType, DBInstance,
                   ...
                   DBWriteBatch};
use libc::{self, c_int, c_uchar, c_void, size_t, uint64_t};
...
pub trait SstFileReaderHandler {
    fn handle_kv(&self, s1: &[u8], s2: &[u8], seq_no: u64, t: u8);
    fn handle_info(&self, s: &[u8]);
    fn handle_err(&self, s: &[u8]);
}

#[repr(C)]
pub struct SstFileReaderHandlerProxy {
    handler: Box<SstFileReaderHandler>,
}

pub struct DefaultSstFileReaderHandler {}

impl SstFileReaderHandler for DefaultSstFileReaderHandler {
    fn handle_kv(&self, _: &[u8], _: &[u8], _: u64, _: u8) {}
    fn handle_info(&self, _: &[u8]) {}
    fn handle_err(&self, _: &[u8]) {}
}

extern "C" fn kv_handler(
    handler: *mut c_void,
    s1: *const u8,
    l1: size_t,
    s2: *const u8,
    ...
    unsafe {
        let handler = &mut *(handler as *mut SstFileReaderHandlerProxy);
        let k1 = slice::from_raw_parts(s1, l1);
        let k2 = slice::from_raw_parts(s2, l2);
        handler.handler.handle_kv(k1, k2, seq_no, t);
    }
}

extern "C" fn info_handler(handler: *mut c_void, s1: *const u8, l1: size_t) {
    unsafe {
        let handler = &mut *(handler as *mut SstFileReaderHandlerProxy);
        let k = slice::from_raw_parts(s1, l1);
        handler.handler.handle_info(k);
    }
}

extern "C" fn err_handler(handler: *mut c_void, s1: *const u8, l1: size_t) {
    unsafe {
        let handler = &mut *(handler as *mut SstFileReaderHandlerProxy);
        let k = slice::from_raw_parts(s1, l1);
        handler.handler.handle_err(k);
    }
}

extern "C" fn destructor(handler: *mut c_void) {
    unsafe {
        Box::from_raw(handler as *mut SstFileReaderHandlerProxy);
    }
}

/// SstFileReader is used to read sst files that generated by SstFileWriter
...
impl SstFileReader {
    pub fn new(
        ...
        unsafe {
            let proxy = Box::into_raw(Box::new(SstFileReaderHandlerProxy { handler: handler }));
            SstFileReader {
    ...
    pub fn read_sequential(
        ...
        has_to: bool,
        to: &[u8],
        use_from_as_prefix: bool,
    ) -> Result<(), String> {
        unsafe {
            ffi_try!(crocksdb_sstfilereader_read_sequential(
                self.inner,
                num,
                has_from as c_uchar,
                from.as_ptr(),
                ...
                to.as_ptr(),
                to.len(),
                use_from_as_prefix as c_uchar
            ));
            Ok(())
        }
    }

    pub fn get_properties<'a>(&mut self) -> Result<&'a TableProperties, String> {
        unsafe {
            let props = ffi_try!(crocksdb_sstfilereader_read_table_properties(self.inner));
            Ok(TableProperties::from_ptr(props))
        }
    }
}
