...
pub trait TablePropertiesCollectorFactory {
    fn create_table_properties_collector(&mut self,
...
extern "C" fn create_table_properties_collector(collector_factory: *mut c_void,
                                                context: u32)
                                                -> *mut c_void {
    unsafe {
        // println!("enter create_table_properties_collector");
        let collector_factory =
            &mut *(collector_factory as *mut TablePropertiesCollectorFactoryProxy);
        let proxy = Box::new(TablePropertiesCollectorProxy {
            name: collector_factory.name.clone(),
            collector: collector_factory.collector_factory
                .create_table_properties_collector(context),
        });
        // println!("leave create_table_properties_collector");
...
pub unsafe fn new_table_properties_collector_factory
    ...
    let collector_factory = crocksdb_ffi::crocksdb_tablepropertiescollectorfactory_create(
                                                    proxy as *mut c_void,
                                                    destructor,
